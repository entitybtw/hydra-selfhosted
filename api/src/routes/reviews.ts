...
export async function reviewsRoutes(app: FastifyInstance) {
  app.get("/games/:shop/:objectId/reviews/check", { preHandler: requireAuth },
  ...
  app.get("/games/:shop/:objectId/reviews",
    async (req: FastifyRequest<{ Params: { shop: string; objectId: string }; Querystring: { take?: string; skip?: string; sortBy?: string } }>) => {
      const { shop, objectId } = req.params;
  ...
  app.post("/games/:shop/:objectId/reviews", { preHandler: requireAuth },
    async (req: FastifyRequest<{ Params: { shop: string; objectId: string }; Body: { reviewHtml?: string; score?: number } }>, reply: FastifyReply) => {
      ...
      const id = randomUUID();
      db.prepare("INSERT OR REPLACE INTO reviews(id, user_id, object_id, shop, review_html, score) VALUES(?,?,?,?,?,?)").run(id, user.id, objectId, shop, reviewHtml, score);
      return formatReview(db.prepare("SELECT * FROM reviews WHERE id = ?").get(id), user.id);
    }
  );

  app.delete("/games/:shop/:objectId/reviews/:reviewId", { preHandler: requireAuth },
    async (req: FastifyRequest<{ Params: { shop: string; objectId: string; reviewId: string } }>) => {
      const user = (req as any).user;
      db.prepare("DELETE FROM reviews WHERE id = ? AND user_id = ?").run(req.params.reviewId, user.id);
      return { ok: true };
    }
  );

  app.put("/games/:shop/:objectId/reviews/:reviewId/:voteType", { preHandler: requireAuth },
    async (req: FastifyRequest<{ Params: { shop: string; objectId: string; reviewId: string; voteType: string } }>) => {
      const user = (req as any).user;
  ...
  app.post("/games/:shop/:objectId/reviews/:reviewId/answers", { preHandler: requireAuth },
    async (req: FastifyRequest<{ Params: { shop: string; objectId: string; reviewId: string }; Body: { answerHtml?: string } }>) => {
      const user = (req as any).user;
      const id = randomUUID();
      db.prepare("INSERT INTO review_answers(id, review_id, user_id, answer_html) VALUES(?,?,?,?)").run(id, req.params.reviewId, user.id, req.body?.answerHtml ?? "");
      return formatAnswer(db.prepare("SELECT * FROM review_answers WHERE id = ?").get(id), user.id);
    }
  );

  app.put("/games/:shop/:objectId/reviews/:reviewId/answers/:answerId/:voteType", { preHandler: requireAuth },
    async (req: FastifyRequest<{ Params: { shop: string; objectId: string; reviewId: string; answerId: string; voteType: string } }>) => {
      const user = (req as any).user;
  ...
  app.delete("/games/:shop/:objectId/reviews/:reviewId/answers/:answerId", { preHandler: requireAuth },
    async (req: FastifyRequest<{ Params: { shop: string; objectId: string; reviewId: string; answerId: string } }>) => {
      const user = (req as any).user;
      db.prepare("DELETE FROM review_answers WHERE id = ? AND user_id = ?").run(req.params.answerId, user.id);
      return { ok: true };
    }
  );
}
